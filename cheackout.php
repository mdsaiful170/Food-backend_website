...
<body>
    ...
    <div class="header-text">
        <h3 class="wow animate__flipInY">cheackout</h3>
        <p><a href="home.php">home</a> <span>/cheackout</span></p>
    </div>

    <section class="checkout">

        <h3 class="titel tlt" data-in-effect="tada" data-out-effect="tada">order summary</h3>

        <form action="" class="form wow animate__zoomIn" method="post" >

            <div class="cart-items">
                <h3>cart itmes</h3>
                <p><span class="name-1">dilicious food 01</span> <span class="price">$3</span></p>
                <p><span class="name-1">dessert food 01</span> <span class="price">$4</span></p>
                <p><span class="name-1">soft drink 03</span> <span class="price">$3</span></p>
                <p class="total"><span class="name-2">grand total :</span><span class="price-2">$10</span></p>
                <a href="cart.php" class="btn-1">view cart</a>
            </div>
            <?php
            ...
            <div class="user-info">
                <h3>Your info</h3>
                <p class="p-1"><i class="fas fa-user"></i>
                    <?php echo $row['name'] ?>
                </p>
                <p class="p-1"><i class="fas fa-envelope"></i>
                    <?php echo $row['email'] ?>
                </p>
                <p class="p-1"><i class="fas fa-phone"></i>
                    <?php echo $row['number'] ?>
                </p>
                <a href="update_profile.php" class="btn-1">upadet profile</a>
                <h3 class="addres">delivary address</h3>
                <p class="p">
                    <i class="fas fa-map-marker-alt"></i>
                    <?php $sql = 'select*from address_table';
                    $result = $connection->query($sql);
                    $row = mysqli_fetch_array($result);
                    ?>
                    <span> <?php echo $row['flat_no'] ?></span>,
                    <span> <?php echo $row['area'] ?></span>,
                    <span> <?php echo $row['city_name'] ?></span>-
                    <span> <?php echo $row['pin_code'] ?></span>
                </p>
                <a href="update_addres.php" class="btn-1">update address</a>

                <select name="method" class="box-sell" required="">
                    <option value="" disabled select>select paymet method</option>
                    <option value="cash on delivary">cash on delivary</option>
                    <option value="credit card"> credit card</option>
                    <option value="patyem">paytem</option>
                    <option value="paypal">paypal</option>
                </select>
                <input type="submit" value="place order" naem="btn" class="btn-1 order-now">
            </div>
        </form>
    ...
    <div class="delete wow  animate__rubberBand">
        <a href="#" class="delete-btn" onclick="return confirm('delete all form cart?');">delete all</a>
    </div>
